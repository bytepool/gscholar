...
def main():
    ...
    if options.version:
        print(acm.__VERSION__)
        return
    if options.output == 'bibtex':
        outformat = acm.FORMAT_BIBTEX
    elif options.output == 'endnote':
        outformat = acm.FORMAT_ENDNOTE
    elif options.output == 'refman':
        outformat = acm.FORMAT_REFMAN
    elif options.output == 'wenxianwang':
        outformat = acm.FORMAT_WENXIANWANG
    if len(args) != 1:
    ...
    if os.path.exists(args):
        ...
        pdfmode = True
        biblist = acm.pdflookup(args, all, outformat, options.startpage)
    else:
        logger.debug("Assuming you want me to lookup the query: {query}".format(query=args))
        biblist = acm.query(args, outformat, options.all)
    if len(biblist) < 1:
    ...
    if options.rename is True:
        if not pdfmode:
            ...
            acm.rename_file(args, biblist[0])
